...
uniform vec4 side_shine_color : hint_color = vec4(1.0);
uniform float backhalo_size : hint_range(0.0, 1.0) = 0.3;
uniform vec4 backhalo_color : hint_color = vec4(1.0);
...
void light(){
	...
	vec3 shadow_buffer = ALBEDO * step_dark * shadow_darkness;
	...
	vec3 final_light = backhalo * backhalo_color.rgb + basic_toon;
	final_light += side_shine * side_shine_color.rgb;
	...
	DIFFUSE_LIGHT += clamp(clamp(LIGHT_COLOR, 0.0, 1.0) * final_light, 0.0, max_brightness);
